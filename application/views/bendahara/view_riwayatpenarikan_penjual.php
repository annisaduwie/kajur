...
?>
    ...
    <section class="content-header">
      <h2>Riwayat Penarikan Penjual</h2>
    </section>
                    ...
                    <table class="table table-bordered table-striped" id="tabel">
                        <thead>
                            <tr>
                                ...
                                <td><b><center>Total Saldo</center></b></td> 
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
        ...
        <div class="modal-dialog" style="text-align: center;">
          ...
          <div class="modal-content">
            <div class="modal-header">
              ...
              <div class="modal-title" style="font-size: 16px;"><strong>Riwayat Penarikan</strong></div>
            </div>
            <div class="modal-body">
                <form id="form">
                  <div class="form-group">
                    <label class="labelform" for="username_pj">Username Penjual</label>
                    <select id="username_pj" name="username_pj" class="form-control" required>
                        <option value="" disabled selected><i>---Pilih Username---</i></option>
                                <?php $dataPenjual=$this->db->query("SELECT username_pj FROM penjual")->result();
                               
                                    foreach ($dataPenjual as $d)  { ?>
                                <option value="<?php echo $d->username_pj;?>"><?php echo $d->username_pj;?></option>
                                <?php } ?>
                                
                            </select>
                  </div>
                  <div class="form-group">
                    <label class="labelform" for="nominal_p">Nominal</label>
                    <input type="number" class="form-control" id="nominal_p" name="nominal_p" placeholder="Masukkan Nominal">
                  </div>
                </form>    
            </div>
...
<script type="text/javascript">
    ...
    function reload() {
        $.ajax({
            url: "<?php echo base_url("riwayatpenarikan_penjual")?>",
            ...
            success:function(data){
                ...
                for(var i in res){
                   ...
                   //first approach to add data (not flexible)
                    pmb.append('<td>'+res[i].id_rp+'</td>');
                    pmb.append('<td>'+res[i].username_pj+'</td>');
    ...
    function tambah_data() {
        ...
        $.ajax({
            url : "<?php echo base_url("riwayatpenarikan_penjual/index_post"); ?>",
            type: "POST",
            data: $('#form').serialize(),
            dataType: "JSON",
            success: function(data)
            ...
            {
                alert("Data gagal ditambahkan atau saldo tidak mencukupi !");
                $('#butSave').text("Simpan");
                $('#butSave').attr('disabled',false); 
            }
        });
